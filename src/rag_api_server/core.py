...
from rag_core.embedding.model import initialize_embedding_model, embed_texts, embed_query
...
class RAGCore:
    ...
    async def process_directory(self, directory_path: str, glob_pattern: str = "**/*[.md|.txt]") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not documents:
                return {"status": "no_documents", "message": "No documents found in the specified directory"}

            # ドキュメントの分割
            print("Splitting documents into chunks...")
            chunks = split_documents(documents)
            if not chunks:
                return {"status": "no_chunks", "message": "No chunks created from the documents"}
    ...
    async def search(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            formatted_results = [
                {
                    "text": text,
                    "similarity": float(score),  # np.float32をJSONシリアライズ可能なfloatに変換
                }
