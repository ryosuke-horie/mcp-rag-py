...
try:
    ...
    # テーブル作成とデータ挿入
    con.execute(
        f"CREATE TABLE items (id INTEGER, embedding FLOAT[{vector_dim}])"
    )  # Revert back to FLOAT[vector_dim] for HNSW index
    # executemany を使用して効率化
    data_to_insert = list(
        zip(ids, vectors.tolist(), strict=False)
    )  # numpy 配列をリストに変換
    ...
    sql = "SELECT id FROM items ORDER BY embedding <-> ?::FLOAT[] LIMIT ?"
